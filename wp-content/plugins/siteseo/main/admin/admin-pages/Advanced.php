...
function siteseo_breadcrumbs_settings_header() {
   echo '<p>'.esc_html__('Breadcrumbs work as a navgation tool for the users, they help user know their current location and provides quick links to their previous browsing path, helping them locate themselves on the website.', 'siteseo').'</p>';
}

function siteseo_breadcrumbs_enable() {
    $options = get_option('siteseo_advanced_option_name', []);
    $enabled = !empty($options) && isset($options['breadcrumbs_enable']) ? true : false;
...
function siteseo_breadcrumbs_display_methods(){
	echo '
<div class="siteseo-inner-tabs-wrap">
	...
	<ul class="siteseo-inner-tabs">
		<li class="siteseo-inner-tab"><label for="siteseo-breadcrumbs-gutenberg"><span class="dashicons dashicons-block-default"></span> Gutenberg Blocks</label></li>
		<li class="siteseo-inner-tab"><label for="siteseo-breadcrumbs-shortcode"><span class="dashicons dashicons-shortcode"></span> Shortcode</label></li>
		<li class="siteseo-inner-tab"><label for="siteseo-breadcrumbs-php"><span class="dashicons dashicons-editor-code"></span> PHP Code</label></li>
	</ul>
	...
	<div class="siteseo-inner-tab-content">
		<h4>Shortcode</h4>
		<p>'.esc_html__('WordPress shortcodes are shortcuts ([shortcode]) that insert features without coding. You can use these shortcodes with Classic Editor, Guteneberg or anyother editor.
		What you need to do is copy the shortcode below and use it in the editor.', 'siteseo').'
	...
	<div class="siteseo-inner-tab-content">
		<h4>PHP Code</h4>
		<p>'.esc_html__('You can add the breadcrumbs by directly adding PHP code, for that make sure you are aware of what you are doing. Use the code below anywhere in your theme', 'siteseo').'</p>
		<pre>'.esc_html("<?php if(function_exists('siteseo_render_breadcrumbs')){
			echo siteseo_render_breadcrumbs();
		}?>").'</pre>
	</div>
</div>
';
...
function siteseo_breadcrumbs_seperator_callback(){
	
	$options = get_option('siteseo_advanced_option_name', []);

	$seperators = ['←','-', '|', '/', '←', '→', '↠', '⇒', '►', '—', '•', '»', '›', '–'];
	
	echo '<div class="siteseo_breadcrumbs_seperator_callback">
		<div class="siteseo-seperator-btns">';
		foreach($seperators as $seperator){
			$checked = '';
			if(!empty($options['breadcrumbs_seperator']) && $seperator == $options['breadcrumbs_seperator']){
				$checked = 'checked';
			}
			
			echo '<label>
			<input type="radio" name="siteseo_advanced_option_name[breadcrumbs_seperator]" value="'.esc_attr($seperator).'" '.esc_attr($checked).'/>
			'.esc_html($seperator). '</label>';
		}

		echo '</div>
		<input type="text" style="width:200px" name="siteseo_advanced_option_name[breadcrumbs_custom_seperator]" placeholder="'.esc_html__('Custom Seperator', 'siteseo').'" value="'.(!empty($options['breadcrumbs_custom_seperator']) ? esc_attr($options['breadcrumbs_custom_seperator']) : '').'"/>
	</div>';
}

function siteseo_breadcrumbs_home_callback(){
	$options = get_option('siteseo_advanced_option_name', []);
	$check = isset($options['breadcrumbs_home']);
	
	echo '<div>
		<label style="margin:10px 0;">
			<input type="checkbox" name="siteseo_advanced_option_name[breadcrumbs_home]" '.esc_html(checked($check, true, false)).'/>
			'.esc_html__('Hide Home', 'siteseo').'
		</label>
		<label>
			<input type="text" name="siteseo_advanced_option_name[breadcrumb_home_label]" placeholder="'.esc_html__('Homepage label', 'siteseo').'" value="'.(!empty($options['breadcrumb_home_label']) ? esc_attr($options['breadcrumb_home_label']) : esc_html__('Home')).'"/>
...
function siteseo_breadcrumbs_prefix_callback(){
	$options = get_option('siteseo_advanced_option_name', []);
	$check = isset($options['breadcrumbs_home']);
	
	echo '<div>
	<label>
	<input type="text" id="siteseo_breadcrumbs_prefix" name="siteseo_advanced_option_name[breadcrumb_prefix]" placeholder="'.esc_html__('Breadcrumb Prefix').'" value="'.(!empty($options['breadcrumb_prefix']) ? esc_attr($options['breadcrumb_prefix']) : '').'"/>
	</label>
	</div>';
}
...
// Breadcrumbs SECTION =======================================================================
add_settings_section('siteseo_advanced_breadcrumbs_section', '', 'siteseo_breadcrumbs_settings_header', 'siteseo-advanced-settings-breadcrumbs');

add_settings_field('siteseo_breadcrumbs_enable', __('Enable Breadcrumbs', 'siteseo'), 'siteseo_breadcrumbs_enable', 'siteseo-advanced-settings-breadcrumbs', 'siteseo_advanced_breadcrumbs_section');

add_settings_field('siteseo_breadcrumbs_display_methods', __('Display Methods', 'siteseo'), 'siteseo_breadcrumbs_display_methods', 'siteseo-advanced-settings-breadcrumbs', 'siteseo_advanced_breadcrumbs_section');

add_settings_field('siteseo_breadcrumbs_seperator', __('Seperator', 'siteseo'), 'siteseo_breadcrumbs_seperator_callback', 'siteseo-advanced-settings-breadcrumbs', 'siteseo_advanced_breadcrumbs_section');

add_settings_field('siteseo_breadcrumbs_home', __('Home Options', 'siteseo'), 'siteseo_breadcrumbs_home_callback', 'siteseo-advanced-settings-breadcrumbs', 'siteseo_advanced_breadcrumbs_section');

add_settings_field('siteseo_breadcrumbs_prefix', __('Breadrumb prefix', 'siteseo'), 'siteseo_breadcrumbs_prefix_callback', 'siteseo-advanced-settings-breadcrumbs', 'siteseo_advanced_breadcrumbs_section');
...
settings_fields('siteseo_advanced_option_group'); ?>
	...
	<div class="siteseo-tab <?php if ('tab_siteseo_advanced_breadcrumbs' == $current_tab) {
        echo 'active';
    } ?>" id="tab_siteseo_advanced_breadcrumbs"><?php do_settings_sections('siteseo-advanced-settings-breadcrumbs'); ?>
        </div>
